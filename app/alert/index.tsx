...
import { retrieveAlert } from "@/constants/controller";
import { AlertInterface } from "@/constants/interface";
...
export default function Alert() {
  ...
  useEffect(() => {
    setChangedFalse();
    (async () => {
      const datasTmp = await retrieveAlert();
      ...
      {datas && datas.length > 0 ? (
        <ScrollView style={styles.scroll}>
          {datas?.map((data, index) => {
            return (
              <TouchableOpacity style={styles.itemContainer} key={data.id}>
                <Image
